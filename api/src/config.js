...
const argv = yargs(hide_bin(process.argv))
  ...
  .option("make-config", {
    ...
    type: "boolean",
    describe:
      "create config file and populate defaults if it does not already exist",
  }).argv;
...
const options = [
  ...
  },
  {
    ...
    default: "https://github.com/ragrag/piston/releases/download/0.1/index",
    validators: [],
  },
];
...
options.for_each((option) => {
  ...
  if (cfg_val === undefined) {
    errored = true;
    logger.error(
      `Config key ${option.key} does not exist on currently loaded configuration`
    );
    return;
  }
